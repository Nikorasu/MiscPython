...
class ScreenArray():
    ...
    def callback(self, indata, frames, time, status):
        ...
        #line = map(lambda x: GrayScale[int(np.clip(x, 0, 1) * (len(GrayScale) - 1))], magnitude[0:Height])
        #self.array[:, -1] = line # add new column on end to hide glitch #np.fromiter(line, dtype=np.int32)
